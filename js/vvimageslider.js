...
(function($) {
    ...
    $.fn.VVImageslider = function(useroptions) {
        ...
        options = $.extend(options, _self.data());

        var $window = $(window);
        var $parentElem = $(_self).parent();

        // the device's resolution and ratio
        var availableHeight;
        var availableWidth;
        var availableRatio;         // lt 1 -> portrait; eq 1 -> square; gt 1 -> landscape
        var maxRatio;
        ...
        // this is the new base element that will replace _self
        ...
        var slidescount = _self.children('vv-slide').length;
        ...
        function buildHTML() {
            ...
            $.each(_self.children('vv-slide'), function(i, img) {
                loadedSlidescount = 0;
                ...
                var imgHref = basepath + filename + '.' + fileext;
                ...
                image.onload = function() {
                    ...
                    vvImagesliderA.append(image);


					console.debug('$image.width():', $image.width());
					console.debug('$image.height():', $image.height());
					maxRatio = Math.min(maxRatio, $image.width() / $image.height());
					console.debug('maxRatio:', maxRatio);

					$image.height(availableWidth * (1 / maxRatio));
                    updateSlidesDimensions(i, vvImagesliderLi);
                    scrollToSlide(options.firstslide);

                    if (loadedSlidescount == slidescount) {
        ...
        function getImageHeight($image) {
			...
			//     console.debug('IF');
			//     height = $image.width() / availableWidth * height;
			// }
			return height;
        };
        ...
        function replaceHTML() {
            // wordpress hack
            if (_self.parent().is('p')) {
                _self.parent().replaceWith(vvImageslideElem);
            } else {
                _self.replaceWith(vvImageslideElem);
            }
        ...
        function updateSlidesDimensions(i, currentLi) {
            ...
            var itemMargin = i < slidescount-1 ? options.margin : 0;
        ...
        function openLightbox(imgUrl) {
			...
			$body.append(lightboxElem);
			$body.append(lightboxBackdropElem);
        };
